...
<div class="container mx-auto p-2">
    ...
    <header class="flex justify-between items-center">
        ...
        <div class="flex items-center">
            <svg width="50px" height="52px" viewBox="0 0 256 264" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid">
                <g>
        ...
        <div class="text-lg hidden md:flex space-x-6">
            <p>Logged as: <a href="{{ route('dashboard') }}" class="hover:text-stone-500">John smith</a></p>
            <form method="post" action="{{route('logout')}}">
                <button type="submit" class="tracking-widest hover:text-stone-500">Logout</button>
            </form>
            <a href="{{route('posts.create')}}" class="inline font-bold text-sm px-6 py-2 text-white rounded-full bg-red-500 hover:bg-red-600">New blog post</a>
            <a href="{{route('login')}}" class="tracking-widest hover:text-stone-500">Login</a>
    ...
    <div class="md:hidden">
        <div id="mobile-menu" class="flex-col items-center hidden py-8 mt-10 space-y-6 bg-white left-6 right-6 drop-shadow-lg">
            <p>Logged as: <a href="{{ route('dashboard') }}">John smith</a></p>
            <form method="POST" action="{{route('logout')}}">
                <button type="submit" class="tracking-widest hover:text-stone-500">Logout</button>
            </form>
            <a href="{{route('posts.create')}}" class="inline font-bold text-sm px-6 py-2 text-white rounded-full bg-red-500 hover:bg-red-600">New blog post</a>
            <a href="{{route('login')}}" class="tracking-widest hover:text-stone-500">Login</a>
            <a href="{{route('register')}}" class="tracking-widest hover:text-stone-500">Register</a>
        </div>
    </div>
